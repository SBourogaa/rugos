...
def validate_interface(interface: str, logger: logging.Logger) -> bool:
    ...
    try:
        output = run_command(
            f"ip link show {interface}", needs_sudo=False, check=False, logger=logger
        )
            ...
            for iface in interfaces:
                ...
                try:
                    wireless_info = run_command(
                        f"iwconfig {name}", needs_sudo=False, check=False, logger=logger
                    )
        ...
        for iface in interfaces:
            ...
            if (
                ...
                or any(
                    ...
                    for prefix in ["veth", "br", "docker", "vbox", "vmnet"]
                )
            ):
                continue
...
def clear_rules(
    wifi_interface: str, ethernet_interface: str, logger: logging.Logger
) -> None:
    """
...
def check_status(logger: logging.Logger) -> None:
    ...
    try:
        ip_forward = run_command(
            "cat /proc/sys/net/ipv4/ip_forward", needs_sudo=False, logger=logger
        ).strip()

        nat_rules = run_command(
            "iptables -t nat -L POSTROUTING -n -v", needs_sudo=True, logger=logger
        )

        forward_rules = run_command(
            "iptables -L FORWARD -n -v", needs_sudo=True, logger=logger
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--clear", action="store_true", help="Clear all rules and disable sharing"
    )
    parser.add_argument(
        "--list-interfaces",
        action="store_true",
    ...
    parser.add_argument(
        "--status", action="store_true", help="Check current network sharing status"
    )
    parser.add_argument(
        "--ip-range",
    ...
    parser.add_argument(
        "--backup", metavar="FILE", help="Backup current iptables rules to file"
    )
    parser.add_argument(
        "--restore", metavar="FILE", help="Restore iptables rules from backup file"
    )
    ...
    try:
        ...
        # if user didn't specify --wifi/--ethernet, enforce unambiguous auto-pick
        if not args.wifi:
            if not interfaces["wifi"]:
                raise NetworkSharingError(
                    "No WiFi interfaces detected.\n"
                    "Specify one explicitly with --wifi <iface>."
                )
            if len(interfaces["wifi"]) > 1:
                raise NetworkSharingError(
                    "Multiple WiFi interfaces detected:\n"
                    f"    {', '.join(interfaces['wifi'])}\n"
                    "Refuse to auto-select. Use --wifi <iface>."
                )
        ...
        if not args.ethernet:
            if not interfaces["ethernet"]:
                raise NetworkSharingError(
                    ...
                    "Specify one explicitly with --ethernet <iface>."
                )
            if len(interfaces["ethernet"]) > 1:
                raise NetworkSharingError(
                    "Multiple Ethernet interfaces detected:\n"
                    f"    {', '.join(interfaces['ethernet'])}\n"
                    "Refuse to auto-select. Use --ethernet <iface>."
                )
            args.ethernet = interfaces["ethernet"][0]
            logger.info(f"Auto-detected Ethernet interface:\n    {args.ethernet}")

        # final sanity: verify the chosen interfaces exist
        if not validate_interface(args.wifi, logger):
            raise NetworkSharingError(
                f"WiFi interface does not exist:\n    {args.wifi}"
            )
        if not validate_interface(args.ethernet, logger):
            raise NetworkSharingError(
                f"Ethernet interface does not exist:\n    {args.ethernet}"
            )
